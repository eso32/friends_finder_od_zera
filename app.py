...
from pycaret.clustering import load_model, predict_model, plot_model  # type: ignore
...
all_df["cluster_name"] = all_df["Cluster"].map(lambda x: cluster_names_and_descriptions[x]['name'])

all_df["is_predicted_cluster"] = all_df["Cluster"].apply(
    lambda x: "Twoja grupa" if x == predicted_cluster_id else "Inne"
)

fig = px.histogram(
    all_df,
    x="cluster_name",
    color="is_predicted_cluster",  # używamy nowej kolumny do oznaczenia
    color_discrete_map={           # przypisanie kolorów
        "Twoja grupa": "orange",
        "Inne": "lightgrey"
    }
)
...
fig.update_layout(
    ...
    showlegend=False  # można wyłączyć legendę, jeśli jest zbędna
)
